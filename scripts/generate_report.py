...
md_path = "outputs/reports/HibritML_Proje_Raporu.md"
docx_path = "outputs/reports/HibritML_Proje_Raporu.docx"

md_content = """# Bulanık Üç Değerli Mantık ile Hibrit Makine Öğrenmesi Modeli Geliştirme
...
for line in lines:
    ...
    if line.startswith("# "):
        ...
        if set(row_data) != {""} and not all(c.startswith("-") for c in row_data):
            table_data.append(row_data)
    else:
        # Check if we just finished a table
        if in_table:
            # Render the table
            if table_data:
